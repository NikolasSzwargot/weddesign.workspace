...
import { Colors, ErrorRoutes, ExpenseGroupingMode, HomeRoutes } from '@weddesign/enums';
...
import { Container, BudgetMainWrapper, BudgetMainFrame, InfoTextWrapper, SearchBarWrapper } from './styles';

const BudgetMain = () => {
  const { router } = useRouting();
  const { t } = useTranslation('budget');
  const [searchQuery, setSearchQuery] = useState('');
  const [groupingMode, setGroupingMode] = useState<ExpenseGroupingMode.Categories | ExpenseGroupingMode.Dates>(
  ...
  useEffect(() => {
    if (!isLoadingByDate && !isLoadingByCategories) {
      setListData(groupingMode === ExpenseGroupingMode.Categories ? groupedByCategories : groupedByDate);
    }
    ...
    isLoadingMainLimit || isLoadingByCategories || isLoadingByDate ? (
      ...
      <>
        ...
        <BudgetMainWrapper>
          ...
          <BudgetMainFrame onLongPress={() => console.log('odpalaj edycje kategorii')} activeOpacity={0.5}>
            <BudgetFrame
          ...
          <InfoTextWrapper>
            <Animated.Text
              ...
              percent: Math.round((mainLimitData.totalPlanned / mainLimitData.limit) * 100),
            })}`}</Animated.Text>
          </InfoTextWrapper>
          ...
          <SearchBarWrapper>
            ...
            />
            <IconButton
              Icon={groupingMode === ExpenseGroupingMode.Categories ? Icons.FilterDate : Icons.FilterDollar}
              onPress={() => {
                setGroupingMode(
