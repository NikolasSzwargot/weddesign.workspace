...
import { Container, ErrorArea, FormInputWrapper, GuestFormWrapper, InputRow, Row } from './styles';
          ...
          <FormInputWrapper>
                ...
                render={({ field: { onChange, value }, fieldState: { error } }) => (
                  <>
                    <Input
                      ...
                      maxLength={10}
                    />
                    <ErrorArea>{error && <Text.Error size={14}>{error.message}</Text.Error>}</ErrorArea>
                  </>
                )}
              />
            </InputRow>
            ...
            <Row>
              <Controller
                ...
                control={control}
                render={({ field: { onChange, value } }) => (
                  <CustomSwitch value={value} onChange={() => onChange(!value)} onColor={Colors.LightBlue} />
                )}
              />
            ...
            <Button onPress={handleSubmit(handleSave)}>{t('guestForm.save')}</Button>
          </FormInputWrapper>
        </GuestFormWrapper>

        <CustomOverlay isVisible={isLoadingCreate || isLoadingUpdate} variant={'center'}>
